-- ============================================================================
-- 077 : Notif « course groupe » — user_id = auth quand possible
-- ============================================================================
-- Les RPC 058+ matchent user_id avec auth.uid() OU public.users.id.
-- En privilégiant supabase_auth_id::text pour la colonne, la cloche et les
-- compteurs non lus restent alignés avec la session (cas comptes migrés).
...
BEGIN
  ...
  FOR v_member IN
    SELECT DISTINCT COALESCE(NULLIF(u.supabase_auth_id::text, ''), u.id) AS notify_user_id
...
COMMENT ON FUNCTION public.notify_group_members_new_ride() IS
  'Après INSERT ride GROUP : notif in-app pour chaque membre (hors créateur), user_id = auth UUID si disponible.';
