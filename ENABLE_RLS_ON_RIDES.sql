...
-- Note : Les devis/factures partagés utilisent probablement la table 'quotes' 
-- avec leur propre système d'accès par token, donc pas besoin d'accès anonyme ici
DROP POLICY IF EXISTS "Authenticated users can read published rides" ON public.rides;
CREATE POLICY "Authenticated users can read published rides"
ON public.rides FOR SELECT
USING (
  status = 'PUBLISHED' 
  AND auth.uid() IS NOT NULL -- Utilisateur authentifié requis
);
