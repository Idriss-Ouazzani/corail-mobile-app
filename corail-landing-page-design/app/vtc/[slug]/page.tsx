...
export default async function VtcProfilePage({
  ...
  const { data: profile, error } = await supabase
    .from("vtc_profiles")
    .select("user_id, slug, display_name, bio, photo_url, phone, whatsapp, email, services, zone_city, zone_radius_km, vehicle_brand, vehicle_model, vehicle_year")
  ...
  const zoneLabel =
    p.zone_city && p.zone_radius_km
      ...
      : p.zone_city || null;
  const vehicleLabel = [p.vehicle_brand, p.vehicle_model, p.vehicle_year].filter(Boolean).join(" ") || null;
